...
bool sse_stream_is_sse_request(const HTTPRequest* request)
{
  ...
  return _strncmp(request->line.method, "GET", 3);
}
...
size_t sse_stream_broadcast_comment(SSEStream* stream, const char* comment)
{
  ...
  size_t comment_len = _strnlen(comment, 256);
...
size_t sse_stream_replay_events(SSEStream* stream, int32_t fd, const char* last_event_id)
{
  ...
  size_t replayed = 0;
  bool   found_id = false;
  size_t id_len   = _strnlen(last_event_id, SSE_EVENT_ID_CAPACITY);
  ...
  for (size_t i = 0; i < stream->queue_size; i++) {
    ...
    if (!found_id) {
      if (_strncmp(stream->event_queue[index].event.id, last_event_id, id_len)) {
        found_id = true;
      }
      continue;
    }
