  ...
  size_t                       key_len)
{
  ...
  size_t header_key_len = _strnlen(header->key, HTTP_HEADER_KEY_CAPACITY);
  if (header_key_len != key_len) {
    return false;
  }

  return _strncmp(header->key, key, key_len);
}
  ...
  size_t                      header_size)
{
  ...
  for (size_t i = 0; i < header_size; i++) {
    if (header_key_matches(&headers[i], key, key_len)) {
      size_t value_len = _strnlen(headers[i].value, HTTP_HEADER_VALUE_CAPACITY);
